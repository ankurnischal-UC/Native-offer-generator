...
const BANKS = [
  { id: "axis", name: "Axis Bank", src: "assets/banks/axis.png" },
  { id: "hdfc", name: "HDFC Bank", src: "assets/banks/hdfc.png" },
  { id: "hsbc", name: "HSBC",      src: "assets/banks/hsbc.png" },
  { id: "yes",  name: "Yes Bank",  src: "assets/banks/yes.png"  },
];
...
const ExchangeIcon = ({ size = 28 }) => (
  <img src="assets/exchange-small.svg" alt="" draggable="false"
    style={{ width: size, height: size, display: "block",
             userSelect: "none", pointerEvents: "none" }}/>
);

const BoltIcon = ({ size = 24 }) => (
  <img src="assets/sameday-small.svg" alt="" draggable="false"
    style={{ width: size, height: size, display: "block",
             userSelect: "none", pointerEvents: "none" }}/>
);
...
const ToteBagIllustration = ({ scale = 1 }) => (
  <img src="assets/merch.svg" alt="" draggable="false"
...
const ExchangeCardWide = ({ exchangeValue }) => (
  <div style={{
    ...
    <div style={{ position: "absolute", right: 16, top: 29 }}>
      <img src="assets/exchange-big.svg" alt="" draggable="false"
...
const SameDayCardWide = () => (
  <div style={{
    ...
    <img src="assets/sameday-big.svg" alt="" draggable="false"
...
// HS SPOTLIGHT 360x360
// Figma: Boxed=Yes, Scheme=Default — light chip with dark text
const HSDiscountBadge = ({ amount, left = 16 }) => (
  <div style={{
    ...
    whiteSpace: "nowrap",
  }}>Up to ₹{INR(amount)} off</div>
);

// Figma: Type=Secondary, Scheme=Dark — dark chip with subtle border
const HSPillButton = ({ children, left = 16 }) => (
  <div style={{
    position: "absolute", left, bottom: 16,
    background: "#fff",
...
// Inner image card (328×328) with badge, title, button — matches Figma frame
const HSSpotlightRO = ({ discount, frameRef }) => (
  <div ref={frameRef} className="asset-frame" style={{
    position: "relative", width: 360, height: 360,
    background: "#fff", overflow: "hidden", borderRadius: 0,
  }}>
    <div style={{
      position: "absolute", left: 16, top: 16,
      width: 328, height: 328, overflow: "hidden", borderRadius: 8,
      background: "#0F0F0F",
    }}>
      <img src="assets/hs-ro.png" alt="" draggable="false"
        style={{ position: "absolute", inset: 0, width: "100%", height: "100%",
                 objectFit: "cover", display: "block", userSelect: "none", pointerEvents: "none" }}/>
      <HSDiscountBadge amount={discount} left={16} />
      <div style={{ position: "absolute", left: 16, top: 44, right: 16,
        fontFamily: "'Open Sauce One', sans-serif",
        fontWeight: 600, fontSize: 24, lineHeight: "32px", color: "#fff" }}>
        Native RO Purifier</div>
      <div style={{ position: "absolute", left: 16, top: 80, right: 16,
        fontFamily: "'Open Sauce One', sans-serif",
        fontWeight: 500, fontSize: 14, lineHeight: "20px", color: "rgba(255,255,255,0.8)" }}>
        Needs no service for 2 years</div>
      <HSPillButton left={16}>Buy now</HSPillButton>
    </div>
  </div>
);

const HSSpotlightLock = ({ discount, frameRef }) => (
  <div ref={frameRef} className="asset-frame" style={{
    position: "relative", width: 360, height: 360,
    background: "#fff", overflow: "hidden", borderRadius: 0,
  }}>
    <div style={{
      ...
      <img src="assets/hs-lock.png" alt="" draggable="false"
        style={{ position: "absolute", inset: 0, width: "100%", height: "100%",
                 objectFit: "cover", display: "block", userSelect: "none", pointerEvents: "none" }}/>
      <HSDiscountBadge amount={discount} left={20} />
      <div style={{ position: "absolute", left: 20, top: 44, right: 16,
        fontFamily: "'Open Sauce One', sans-serif",
        fontWeight: 600, fontSize: 24, lineHeight: "32px", color: "#fff" }}>
        Native<br/>Lock Ultra</div>
      <div style={{ position: "absolute", left: 20, top: 112, right: 16,
        fontFamily: "'Open Sauce One', sans-serif",
        fontWeight: 500, fontSize: 14, lineHeight: "20px", color: "rgba(255,255,255,0.8)" }}>
        Designed to be held.<br/>Built to protect.</div>
      <HSPillButton left={20}>Know more</HSPillButton>
    </div>
  </div>
);
...
const App = () => {
  ...
  const [hsRoDiscount, setHsRoDiscount] = useState("2500");
  const [hsLockDiscount, setHsLockDiscount] = useState("1800");
      ...
      <header className="topbar">
        <div className="brand">
          <div className="brand-logo" style={{display:"flex", alignItems:"center"}}>
            <img src="assets/native-logo.svg" alt="Native"
      ...
      <main className="main">
        ...
        <aside className="panel">
          ...
          <div className="asset-tabs">
            <button className={"asset-tab" + (assetType === "detail" ? " active" : "")}
              onClick={() => setAssetType("detail")}>Detail page</button>
            <button className={"asset-tab" + (assetType === "hs" ? " active" : "")}
              onClick={() => setAssetType("hs")}>HS Spotlight</button>
          </div>
          ...
          {assetType === "hs" ? (
            <>
              <section className="section">
                ...
                <div className="asset-tabs" style={{marginBottom:18}}>
                  <button className={"asset-tab" + (hsProduct === "ro" ? " active" : "")}
                    onClick={() => setHsProduct("ro")}>RO</button>
                  <button className={"asset-tab" + (hsProduct === "lock" ? " active" : "")}
                    onClick={() => setHsProduct("lock")}>Locks</button>
                </div>
                {hsProduct === "ro" ? (
                  <Field label="Enter RO discount">
                    <TextInput type="number" value={hsRoDiscount}
                      onChange={setHsRoDiscount} prefix="₹" placeholder="2,500"/>
                  </Field>
                ) : (
                  <Field label="Enter Lock discount">
                    <TextInput type="number" value={hsLockDiscount}
                      onChange={setHsLockDiscount} prefix="₹" placeholder="1,800"/>
                  </Field>
                )}
              </section>
              ...
              <button className="download-btn"
                onClick={async () => {
                  if (!hsFrameRef.current) return;
                  setDownloading(true);
                  try {
                    const dataUrl = await window.htmlToImage.toJpeg(hsFrameRef.current, {
                      pixelRatio: 4, quality: 0.95, backgroundColor: "#0F0F0F", cacheBust: true,
                    });
                    const a = document.createElement("a");
                    a.href = dataUrl;
                    a.download = `UC-HS-${hsProduct === 'ro' ? 'RO' : 'Lock'}-360.jpg`;
                    a.click();
                    setToast({ ok: true, msg: "Downloaded at 1440×1440px" });
                  } catch (e) {
                    setToast({ ok: false, msg: "Could not export." });
                  } finally {
                    setDownloading(false);
                    setTimeout(() => setToast(null), 3500);
                  }
                }} disabled={downloading}>
                {downloading ? <><span className="spinner"/>Generating…</> : <>
              ...
              <div className="export-note">Exports the selected asset at 1440×1440px (4×).</div>
            </>
          ) : (
            <></>
          )}
        ...
        <section className="preview-area">
          <div className="preview-header">
            <div>
              ...
              <div className="preview-meta">
                {assetType === "hs"
                  ? `HS Spotlight · ${hsProduct === "ro" ? "RO" : "Lock"} · 360×360px`
                  : `${tpl.label.replace(/^[0-9]\.\s*/, '')} · ${tpl.w}×${tpl.h}px`}
              </div>
            </div>
            <div className="preview-chips">
              {assetType === "hs" ? (
                <span className="chip">HS SPOTLIGHT</span>
              ) : (
                <>
          ...
          <div className="canvas-bg">
            ...
            <div className="canvas-stage">
              {assetType === "hs" ? (
                hsProduct === "ro" ? (
                  <HSSpotlightRO discount={Number(hsRoDiscount)||0} frameRef={hsFrameRef}/>
                ) : (
                  <HSSpotlightLock discount={Number(hsLockDiscount)||0} frameRef={hsFrameRef}/>
                )
              ) : renderTemplate()}
            </div>
            {toast && (
